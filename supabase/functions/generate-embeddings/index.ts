 ...
 */
async function storeEmbedding(client: any, receiptId: string, contentType: string, embedding: number[], metadata: Record<string, any> = {}) {
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { receiptId, contentType, content, metadata, model, processAllFields } = requestBody;
